  ...
  const {theme , setTheme} = useContext(ThemeContext);
    return(
        <div className={`${Styles.countryItem}  ${Styles[theme]}`}>
          <Link to={`/country/${props.country.alpha3Code}`} className={Styles[theme]}>
            <div className={Styles.imgArea}>
                <img src={props.country.flags.png} alt="Bandeira do Pais" className="img"/>
            </div>
            <div className={Styles.countryData}>
                <div className={Styles.countryName}>{props.country.name}</div>
                <p>Population :<span> {props.country.population.toLocaleString("en")} </span></p>
